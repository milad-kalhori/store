...
import {loginApi, uploadUserPhoto} from "../../api/api_auth";
import {toast} from "react-toastify";
import {useTranslation} from "react-i18next";


export const User = ({name, id, img}) => {
  ...
  return <ButtonBase style={{width: "100%"}}><Grid container direction={"row"} className={classes.goodsParent}>
    <img src={getImage()} className={classes.goodsImg}/>
    <Grid item container direction={"column"} style={{width: 'max-content'}} alignItems={"flex-start"}
          className={classes.goodsNameParent}>
...
const LeftSidebar = () => {
  ...
  return (
    <div className={classes.root}>
      ...
      <Grid item container direction={"column"} className={classes.goodsRoot}>
        <Typography className={classes.goodsTitle}>
          {t("userListTitle")}
        </Typography>
        <Divider style={{marginLeft: -24, marginRight: -24}}/>
        {
          users.map((item, index) => {
            return (<Link to={`/users/${item._id}/${item.name}`} style={{width: "100%"}}>
              <User name={item.name} id={item.username} img={item.image}/>
...

        {
          staff.map(() => {
            if(localStorage.getItem("role") === "staff")
              return  <MenuItem onClick={() => {
                window.location.assign('http://localhost:3000/staff');
              }}>{t("staffPage")}</MenuItem>
          })
        }
